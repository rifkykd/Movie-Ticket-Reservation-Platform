...
<div class="page">
    <div class="page-title"><i class="bi bi-pencil-square"></i> Edit Showtime</div>
    <div class="form-card">
        <form action="${pageContext.request.contextPath}/showtime/edit" method="post">
            <input type="hidden" name="showtimeId"  value="${showtime.showtimeId}">
            <input type="hidden" name="bookedSeats" value="${showtime.bookedSeats}">
            ...
            <div class="mb-3">
                ...
                <div class="input-box">
                    ...
                    <c:choose>
                                ...
                                <c:forEach var="m" items="${movies}">
                                    <c:choose>
                                        ...
                                        <c:when test="${m.status=='Now Showing'}">
                                            <option value="${m.title}" ${m.title==showtime.movieTitle?'selected':''}>${m.title} ● Now Showing</option>
                                        </c:when>
                                        <c:otherwise>
                                            <option value="${m.title}" ${m.title==showtime.movieTitle?'selected':''}>${m.title} ○ Coming Soon</option>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                            </select>
                        ...
                        <c:otherwise>
                            <input type="text" name="movieTitle" value="${showtime.movieTitle}" required>
                        </c:otherwise>
                    </c:choose>
                </div>
            </div>
                ...
                <div class="input-box">
                    ...
                    <select name="hallId" required>
                        <c:forEach var="h" items="${halls}">
                            <option value="${h.hallId}" ${h.hallId==showtime.hallId?'selected':''}>${h.hallName} (${h.totalSeats} seats)</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
            ...
            <div class="row g-3 mb-3">
                <div class="col-6">
                    ...
                    <div class="input-box"><span class="input-icon"><i class="bi bi-calendar3"></i></span><input type="date" name="showDate" value="${showtime.showDate}" required></div>
                </div>
                <div class="col-6">
                    <label class="field-label">Show Time</label>
                    <div class="input-box"><span class="input-icon"><i class="bi bi-clock"></i></span><input type="time" name="showTime" value="${showtime.showTime}" required></div>
                </div>
            </div>
                ...
                <div class="input-box">
                    ...
                    <select name="showType" id="showType" onchange="updatePreview()" required>
                        <c:forEach var="t" items="${showTypes}">
                            <option value="${t.name()}" ${t.name()==showtime.showType.name()?'selected':''}>${t.label}</option>
                        </c:forEach>
                    </select>
                </div>
            ...
            <div class="mb-3">
                ...
                <div class="input-box"><span class="input-icon"><i class="bi bi-currency-dollar"></i></span><input type="number" name="basePrice" id="basePrice" value="${showtime.basePrice}" min="100" step="50" oninput="updatePreview()" required></div>
            </div>
            <div class="price-preview">Final price per seat: <strong id="priceOut">LKR —</strong></div>

            <div class="d-flex gap-3 mt-4">
                <button type="submit" class="btn-submit"><i class="bi bi-save2"></i> Save Changes</button>
